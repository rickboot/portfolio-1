...
export default function Nav() {
  ...
  return (
    <div className='relative z-10'>
      <motion.div
        className='rgba(var(--daphne-blue), var(5-)); fixed left-1/2 top-0 
        h-[4.5rem] 
        w-full rounded-none
        bg-[--daphne-blue]
      ...
      <nav className='fixed left-1/2 top-[0.15rem] flex h-12 -translate-x-1/2 py-2 sm:top-6 sm:h-[initial] sm:py-0'>
        <ul className='relative flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-[--raven-black] sm:w-[initial] sm:flex-nowrap sm:gap-5'>
          {links.map((link) => {
            return (
              <motion.li
